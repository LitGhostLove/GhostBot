...
`.creatvoichanl` `.cvch` | 以使用者提供的名稱創建一個語音頻道。 **機器人需要`管理頻道`權限** | `.cvch VoiceChannelName`
`.deltxtchanl` `.dtch` | 刪除指定的文字頻道。 **機器人需要`管理頻道`權限** | `.dtch TextChannelName`
...
`.unsetmusicchannel` `.usmch` | 機器人將於第一次點播歌曲的頻道輸出播放、結束、暫停和刪除等音樂訊息。 **機器人需要`管理訊息`權限** | `.usmch`
...
`.gencurrency` `.gc` | Toggles currency generation on this channel. Every posted message will have chance to spawn currency. Chance is specified by the Bot Owner. (default is 2%) **機器人需要`管理訊息`權限** | `.gc`
`.poll` `.ppoll` | Creates a public poll which requires users to type a number of the voting option in the channel command is ran in. **機器人需要`管理訊息`權限** | `.ppoll Question?;Answer1;Answ 2;A_3`
`.pollstats` | Shows the poll results without stopping the poll on this server. **機器人需要`管理訊息`權限** | `.pollstats`
`.pollend` | Stops active poll on this server and prints the results in this channel. **機器人需要`管理訊息`權限** | `.pollend`
