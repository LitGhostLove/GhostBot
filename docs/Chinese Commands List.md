...
`.hgit` | 生成 commandlist.md 檔案。**僅限Bot所有者** | `.hgit`
...
`.donadd` | 將一名贊助者添加到資料庫。 **僅限Bot所有者** | `.donadd Donate Amount`
...
`.logserver` | 開啟或關閉紀錄伺服器所有事件，開啟後會將所有事件紀錄到當前文字頻道。 **Bot需要`管理員`權限** **僅限Bot所有者** | `.logserver enable` 或 `.logserver disable`
`.logignore` | Toggles whether the `.logserver` command ignores this channel. Useful if you have hidden admin channel and public log channel. **Bot需要`管理員`權限** **僅限Bot所有者** | `.logignore`
`.logevents` | 列出您可以使用`.log`紀錄的所有事件類型。 **Bot需要`管理員`權限** **僅限Bot所有者** | `.logevents`
`.log` | Toggles logging event. Disables it if it is active anywhere on the server. Enables if it isn't active. Use `.logevents` to see a list of all events you can subscribe to. **Bot需要`管理員`權限** **僅限Bot所有者** | `.log userpresence` 或 `.log userbanned`
`.migratedata` | Migrate data from old bot configuration **僅限Bot所有者** | `.migratedata`
...
`.rotateplaying` `.ropl` | Toggles rotation of playing status of the dynamic strings you previously specified. **僅限Bot所有者** | `.ropl`
`.addplaying` `.adpl` | Adds a specified string to the list of playing strings to rotate. Supported placeholders: `%servers%`, `%users%`, `%playing%`, `%queued%`, `%time%`, `%shardid%`, `%shardcount%`, `%shardguilds%`. **僅限Bot所有者** | `.adpl`
`.listplaying` `.lipl` | Lists all playing statuses with their corresponding number. **僅限Bot所有者** | `.lipl`
`.removeplaying` `.rmpl` `.repl` | Removes a playing string on a given number. **僅限Bot所有者** | `.rmpl`
...
`.scadd` | Adds a command to the list of commands which will be executed automatically in the current channel, in the order they were added in, by the bot when it startups up. **僅限Bot所有者** | `.scadd .stats`
`.sclist` | Lists all startup commands in the order they will be executed in. **僅限Bot所有者** | `.sclist`
`.wait` | Used only as a startup command. Waits a certain number of miliseconds before continuing the execution of the following startup commands. **僅限Bot所有者** | `.wait 3000`
`.scrm` | Removes a startup command with the provided command text. **僅限Bot所有者** | `.scrm .stats`
`.scclr` | Removes all startup commands. **僅限Bot所有者** | `.scclr`
`.fwmsgs` | Toggles forwarding of non-command messages sent to bot's DM to the bot owners **僅限Bot所有者** | `.fwmsgs`
`.fwtoall` | Toggles whether messages will be forwarded to all bot owners or only to the first one specified in the credentials.json file **僅限Bot所有者** | `.fwtoall`
`.leave` | Makes Nadeko leave the server. Either server name or server ID is required. **僅限Bot所有者** | `.leave 123123123331`
`.die` | Shuts the bot down. **僅限Bot所有者** | `.die`
`.setname` `.newnm` | Gives the bot a new name. **僅限Bot所有者** | `.newnm BotName`
...
`.setstatus` | Sets the bot's status. (Online/Idle/Dnd/Invisible) **僅限Bot所有者** | `.setstatus Idle`
`.setavatar` `.setav` | Sets a new avatar image for the NadekoBot. Argument is a direct link to an image. **僅限Bot所有者** | `.setav http://i.imgur.com/xTG3a1I.jpg`
`.setgame` | Sets the bots game. **僅限Bot所有者** | `.setgame with snakes`
`.setstream` | Sets the bots stream. First argument is the twitch link, second argument is stream name. **僅限Bot所有者** | `.setstream TWITCHLINK Hello`
`.send` | Sends a message to someone on a different server through the bot.  Separate server and channel/user ids with `|` and prefix the channel id with `c:` and the user id with `u:`. **僅限Bot所有者** | `.send serverid|c:channelid message` 或 `.send serverid|u:userid message`
`.reloadimages` | Reloads images bot is using. Safe to use even when bot is being used heavily. **僅限Bot所有者** | `.reloadimages`
...
`.ubl` | Either [add]s or [rem]oves a user specified by a Mention or an ID from a blacklist. **僅限Bot所有者** | `.ubl add @SomeUser` 或 `.ubl rem 12312312313`
`.cbl` | Either [add]s or [rem]oves a channel specified by an ID from a blacklist. **僅限Bot所有者** | `.cbl rem 12312312312`
`.sbl` | Either [add]s or [rem]oves a server specified by a Name or an ID from a blacklist. **僅限Bot所有者** | `.sbl add 12312321312` 或 `.sbl rem SomeTrashServer`
...
`.listglobalperms` `.lgp` | Lists global permissions set by the bot owner. **僅限Bot所有者** | `.lgp`
`.globalmodule` `.gmod` | Toggles whether a module can be used on any server. **僅限Bot所有者** | `.gmod nsfw`
`.globalcommand` `.gcmd` | Toggles whether a command can be used on any server. **僅限Bot所有者** | `.gcmd .stats`
`.resetperms` | 重設這個伺服器上的權限模組至預設值。 **Bot需要`管理員`權限** | `.resetperms`
`.resetglobalperms` | 重設所有伺服器上的權限模組至預設值。 **僅限Bot所有者** | `.resetglobalperms`
...
`.local` `.lo` | 點播開設者電腦內的一首歌曲。僅限Bot擁有者 **僅限Bot所有者** | `.lo C:/music/mysong.mp3`
`.localplaylst` `.lopl` | 點播Bot開設者電腦裡某資料夾內的所有歌曲。 **僅限Bot所有者** | `.lopl C:/music/classical`
...
`.typeadd` | Adds a new article to the typing contest. **僅限Bot所有者** | `.typeadd wordswords`
`.typelist` | Lists added typing articles with their IDs. 15 per page.  | `.typelist` 或 `.typelist 3`
`.typedel` | Deletes a typing article given the ID. **僅限Bot所有者** | `.typedel 3`
...
`.award` | 給予某位成員一定數量的貨幣。您還可以指定一個身分組，已將貨幣贈與該身分組中的所有成員。 **僅限Bot所有者** | `.award 100 @person` 或 `.award 5 Role Of Gamblers`
`.take` | 取走某位成員一定數量的貨幣。 **僅限Bot所有者** | `.take 1 @someguy`
...
`.startevent` | 開始一個貨幣贈送活動。目前僅可使用`reaction`和`sneakygamestatus`。 **僅限Bot所有者** | `.startevent reaction`
...
`.slotstats` | 顯示此機器人的吃角子老虎機統計資訊。 **僅限Bot所有者** | `.slotstats`
`.slottest` | Tests to see how much slots payout for X number of plays. **僅限Bot所有者** | `.slottest 1000`
...
`.autotrans` `.at` | Starts automatic translation of all messages by users who set their `.atl` in this channel. You can set "del" argument to automatically delete all translated user messages. **Bot需要`管理員`權限** **僅限Bot所有者** | `.at` 或 `.at del`
...
`.listservers` | 列出機器人加入的所有伺服器與其資訊。 **僅限Bot所有者** | `.listservers 3`
`.savechat` | 將指定數量的訊息儲存為文字檔並傳給你。 **僅限Bot所有者** | `.savechat 150`
...
`.activity` | 列出以往洗頻的使用者與其詳細資訊。 **僅限Bot所有者** | `.activity`
...
`.activity` | Checks for spammers. **僅限Bot所有者** | `.activity`
`.parewrel` | Forces the update of the list of patrons who are eligible for the reward. **僅限Bot所有者** | `.parewrel`
...
`.remindtemplate` | Sets message for when the remind is triggered.  Available placeholders are `%user%` - user who ran the command, `%message%` - Message specified in the remind, `%target%` - target channel of the remind. **僅限Bot所有者** | `.remindtemplate %user%, do %message%!`
...
`.crstatsclear` | Resets the counters on `.crstats`. You can specify a trigger to clear stats only for that trigger. **僅限Bot所有者** | `.crstatsclear` 或 `.crstatsclear rng`
...
`.nsfwcc` | Clears nsfw cache. **僅限Bot所有者** | `.nsfwcc`
